import { twMerge } from "tailwind-merge"
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement>{
   variant?: "primary" | "danger" | "secondary" | "ghost" | "disabled" | "third",
}

export const Button = ({variant = "primary", className,children, ...rest}: ButtonProps) => {

    const variants = {
        primary: "bg-gray-500/80",
        danger: "bg-red-700",
        secondary: "bg-blue-800",
        ghost: "bg-transparent",
        disabled: "bg-gray-400 cursor-not-allowed",
        third: "bg-green-500"
    };

    return(
        <button 
        className={twMerge(
            `rounded-full px-8 py-2 cursor-pointer sm:text-lg sm:px-10 shadow-md shadow-gray-500/50 transition-transform
             duration-200 hover:scale-95 active:scale-[98%]`,variants[variant], className)}
        {...rest}
        >
            {children}
        </button>
      
    )
}